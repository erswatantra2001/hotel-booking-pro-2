...
//           <input className='input' value={formData.password} type='Password' onChange={handleChange} placeholder='Password' name='password' required></input><br /><br />
...
export default function Signuppage() {
  ...
  const handleSubmit = async(e) => {
    ...
    if (formData.pass === conpass){
      console.log("if constion pass both password are match")
      c= await axios.post("http://localhost:5000/singup",formData)
    }
    else{
      alert ("password did not match")
    }
